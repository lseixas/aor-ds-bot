...
class Rw(commands.Cog):
    ...
    @commands.command(name='rw', help='Prints the ranking table')
    async def rw(self, ctx, p1: discord.Member, p2: discord.Member = None, p3: discord.Member = None):
        ...
        with open(file_path_ranking, "r") as f:
            ...
            for player in [p1, p2, p3]:
                ...
                if player_key not in ranking['players']:
                    ...
                    ranking['players'][player_key]['wins'] += 1
        ...
        await ctx.send(
            f"Result registered: **{1} WIN**\n"
            f"Players: {', '.join(players)}\n"
            f"{table_to_print}"
        )

async def setup(bot):
    await bot.add_cog(Rw(bot))
